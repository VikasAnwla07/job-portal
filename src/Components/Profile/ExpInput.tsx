...
const ExpInput = (props: any) => {
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-10 [&>*]:w-1/2">
                <MonthPickerInput withAsterisk maxDate={endDate || undefined} label="Start Date" placeholder="Pick date" value={startDate?.toISOString().slice(0, 7) || null} onChange={(value) => setStartDate(value ? new Date(value) : null)} />

                <MonthPickerInput disabled={checked} withAsterisk minDate={startDate || undefined} maxDate={new Date()} label="End Date" placeholder={checked ? "Present" : "Pick date"} value={checked ? null : endDate?.toISOString().slice(0, 7) || null} onChange={(value) => setEndDate(value ? new Date(value) : null)} />
